...
sudo zypper -c ${zypp_config} --root=${target} --releasever=/ \
    --non-interactive  --no-gpg-checks install --auto-agree-with-licenses libzypp sudo zypper filesystem sles-release-15 vim
 #sudo zypper -c ${zypp_config} --installroot=${target} clean all
